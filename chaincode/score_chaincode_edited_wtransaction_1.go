...
var transactionIndexStr = "_transindex" //name for the key/value that will store a list of all known transactions


type Company struct {
	Name   string `json:"name"` //the fieldtags are needed to keep case from bouncing around
	Score  int    `json:"score"`
	Ontime int    `json:"ontime"`  //tracks number of ontime transactions for the company 
	Total  int    `json:"total"`   
}

//temporary Transaction struct

type Transaction struct{
	Name string `json: "name"`				// Name for B_comp that is receiving the rating 
	User string `json: "user_small"`  		// user providing feedback on B_comp 
	Ontime int `json: "ontime"`		        // ontime is binary if on time or not , 1 = on time, 0 = not on time 
...
func (t *SimpleChaincode) Invoke(stub shim.ChaincodeStubInterface, function string, args []string) ([]byte, error) {
	...
	} else if function == "init_transaction" { //change owner of a marble
...
type Transaction struct{
	...
	//check if company already exists
	transactionAsBytes, err := stub.GetState(name)  //checking on transaction  name of company Question -- don't we need a transaction id to uniquely identify transaction? 
	if err != nil {
		return nil, errors.New("Failed to get company name in transactions")
	}
	res := Transaction{}
	json.Unmarshal(transactionAsBytes, &res)
	if res.Name == name {
		fmt.Println("This company transaction arleady exists: " + name)  // may need to be changed if we have a unique transaction id
		fmt.Println(res)
		return nil, errors.New("This company transaction arleady exists") //all stop a marble by this name exists
	}

	//   0       1       2     3
	// "Name", "Score, "Ontime", "Total" for company 
	//"Name", "User", "Ontime", "Timestamp" for transaction 

	//build the company transaction json string manually
	str := `{"name": "` + name + `, "user": ` + user + `, "ontime": ` + strconv.Itoa(Ontime) + `, "timestamp": ` + strconv.Itoa(Timestamp) + `}`
	fmt.Println("Company Transaction: " + str)
	err = stub.PutState(name, []byte(str)) //store company with id as key
	if err != nil {
		return nil, err
	}
	...
	//get the company index
	// Had to change err to err1 as when building, received error "no new vairables on left side of :="
	transactionAsBytes, err1 := stub.GetState(transactionIndexStr)
	if err1 != nil {
		return nil, errors.New("Failed to get company transaction index")
	}
	var transactionIndex []string
	json.Unmarshal(transactionAsBytes, &transactionIndex) //un stringify it aka JSON.parse()

	//append
	transactionIndex = append(transactionIndex, name) //add company name to index list
	fmt.Println("! company transaction index: ", transactionIndex)
