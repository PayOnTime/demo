...
type B_Comp struct{
	...
	score int `json:"score"`					// initial score of a buyer , th
	OnTime__n int `json:"count_on_time"`		//number of payments on time
	Total_n int `json:"total_count"`			//total number of transactions
}
...
func (t *SimpleChaincode) init_marble(stub *shim.ChaincodeStub, args []string) ([]byte, error) {
	...
	count_on_time := strconv.Atoi(args[2])
	total_count := strconv.Atoi(args[3])
	...
	res := B_Comp{}   //defining an empty comapny object , of a certain type
