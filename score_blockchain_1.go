...
//BComp initial score of a buyer , th
//number of payments on time
//total number of transactions
type BComp struct {
	Name    string `json:"name"`
	score   int    `json:"score"`
	OnTimeN int    `json:"count_on_time"`
...
func (t *SimpleChaincode) Init(stub *shim.ChaincodeStub, function string, args []string) ([]byte, error) {
	...
	var empty []string
	jsonAsBytes, _ := json.Marshal(empty) //creating datastore to track all marbles, aka companies marshal an emtpy array of strings to clear the index
...
// ============================================================================================================================
func (t *SimpleChaincode) initmarble(stub *shim.ChaincodeStub, args []string) ([]byte, error) {
	var err error

	//   0       1       2     3
	// "name", "score", "count_on_time", "total_count"
	if len(args) != 4 {
	...
	score := strconv.Atoi(args[1])
	countontime := strconv.Atoi(args[2])
	totalcount := strconv.Atoi(args[3])

	//check if marble already exists
	marbleAsBytes, err := stub.GetState(name) //pulled this out of ledger
	if err != nil {
		return nil, errors.New("Failed to get company name")
	}
	res := BComp{}                      //defining an empty comapny object , of a certain type
	json.Unmarshal(marbleAsBytes, &res) //takes byte aray and turns it into object
	if res.Name == name {               // if company already exists, stop this check;to interact with company use res.Name
	...
	//build the company json string manually if company doesn't already exist per above
	str := `{"name": "` + name + `", "score": "` + score + `", "count_on_time": ` + count_on_time + `, "total_count": "` + total_count + `"}`
	err = stub.PutState(name, []byte(str)) // converting to byte array from json format and putting it back on ledger store marble with id as key
	if err != nil {
		return nil, err
	}

	//get the marble index
	marblesAsBytes, err := stub.GetState(companyIndexStr)
	if err != nil {
		return nil, errors.New("Failed to get company index")
	}
	var companyIndex []string
	json.Unmarshal(marblesAsBytes, &companyIndex) //un stringify it aka JSON.parse()

	//append
	companyIndex = append(companyIndex, name) // marshall back into byte array and update the ledger, as this is still in chain code  add marble name to index list
	fmt.Println("! marble index: ", companyIndex)
...
// ============================================================================================================================
func (t *SimpleChaincode) setuser(stub *shim.ChaincodeStub, args []string) ([]byte, error) {
	var err error
...
// ============================================================================================================================
func (t *SimpleChaincode) opentrade(stub *shim.ChaincodeStub, args []string) ([]byte, error) {
	var err error
	var willsize int
	var tradeaway Description
...
// ============================================================================================================================
func (t *SimpleChaincode) performtrade(stub *shim.ChaincodeStub, args []string) ([]byte, error) {
	var err error
...
// ============================================================================================================================
func (t *SimpleChaincode) removetrade(stub *shim.ChaincodeStub, args []string) ([]byte, error) {
	var err error
